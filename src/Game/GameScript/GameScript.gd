...
func _ready() -> void:
	if enemy_scene == null:
		printerr("Warning! No enemy scene exists for ", name, ". Make sure you assign one before trying to spawn an enemy.")
...
func spawn_enemy() -> Enemy:
	if enemy_scene == null:
		printerr("Warning! No enemy scene has been assigned to script ", name, ". Failed trying to spawn an enemy.")
		return null
