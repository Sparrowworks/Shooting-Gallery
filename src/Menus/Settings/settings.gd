...
func _update_sliders() -> void:
	...
	AudioServer.set_bus_volume_db(
		AudioServer.get_bus_index("Master"), linear_to_db(Globals.master_volume / 100)
	)
...
func _on_clear_button_pressed() -> void:
	(
		...
		. show_message(
			"Warning",
			"Warning! This will clear all data you had previously saved. Are you sure you want to do this?",
			"Yes",
			true
		)
	)
